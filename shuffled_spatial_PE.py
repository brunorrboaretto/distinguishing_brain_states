...
fp_out_1 = 'shuffled_spatial_S_raw_ec.npy'
fp_out_2 = 'shuffled_spatial_S_raw_eo.npy'

fp_out_3 = 'shuffled_spatial_S_alpha_ec.npy'
fp_out_4 = 'shuffled_spatial_S_alpha_eo.npy'

raw_ec = []

for i in range(1,110):
    print('shuffled raw EC',i)
    if i not in black_list:
        data = np.load('raw_data/data_ec_%.3d.npy' % i)
        data_aux = np.transpose(data)
        np.random.seed(seed)
        np.random.shuffle(data_aux)
        data = np.transpose(data_aux)
        Aux = []
        Size = data.shape[1]
        Aux = []
